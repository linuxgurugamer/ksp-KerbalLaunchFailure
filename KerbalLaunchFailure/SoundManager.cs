...
namespace KerbalLaunchFailure
{
    class SoundManager
    {
        ...
        public static void LoadSound(string filePath, string soundName)
        {
            if (instance != null)
            {
                Log.Info("LoadSound, filePath: " + filePath + "   soundName: " + soundName);
        ...
        public static void CreateFXSound(Part part, FXGroup group, string defaultSound, bool loop, float maxDistance = 30f)
        {
            if (part == null)
            {
                ...
                //group.audio = this.gameObject.AddComponent<AudioSource>();
            }
            else
            {
                group.audio = part.gameObject.AddComponent<AudioSource>();
            }
            group.audio.volume = 1f; // GameSettings.SHIP_VOLUME;
